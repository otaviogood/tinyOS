<script>
    ...
    const icons = [
        ...
        {name: "Audio", icon: "fa-volume-up", bgcolor: "bg-pink-500", link: "musicplayer"},
        ...
        {name: "Tiny Quest", icon: "fa-hat-wizard", bgcolor: "bg-blue-500", link: "tinyquest/mapscreen"},
        ...
        {#each icons as icon, i}
            <div class="flex-center-all flex-col text-white select-none bXXorder border-red-500">
                <button class="p-2 active:scale-105 transform transition-all duration-75" on:click={() => {push("/" + icon.link)}}>
                    <div class="w-64 h-64 rounded-3xl {icon.bgcolor} flex-center-all text-9xl text-white"><i class="fas {icon.icon}"></i></div>
                    <div class="font-bold text-3xl m-2">{icon.name}</div>
                </button>
            </div>
        {/each}
    </div>
