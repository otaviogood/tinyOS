<script>
    ...
    async function recordButton() {
        ...
        // mic = new Tone.UserMedia({ context });
...
<div class="flex">
    <div>
        {#if !recorder}
            ...
            >
        {:else}
            <button class="fit-full-space bg-purple-600 text-white text-9xl" on:pointerdown={stopButton}>STOP</button>
        {/if}
    </div>
    {#if recording}
        <div>
            <button
                class="{playing
                    ? 'bg-emerald-400'
                    : 'bg-green-600'} active:bg-green-800 text-white text-9xl m-2 p-10 rounded-r-full"
                style="width:26rem"
